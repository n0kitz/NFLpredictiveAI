...
python -m pytest -q                        # 337 tests (slow ~30 min — see GUIDEBOOK §5 "Next")
cd frontend && npm run build && npm test   # tsc + vite build, 57 vitest tests (run from frontend/ — vitest setup needs that cwd)
```
