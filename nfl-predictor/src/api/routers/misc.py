...
from ..schemas import (
    ...
    ValuePick,
    ValuePicksResponse,
    ValuePickHistoryItem,
    ValuePickHistoryResponse,
...
def get_accuracy(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail="seasons must be comma-separated integers, e.g. '2024,2025'",
        )
    if not season_list:
        raise HTTPException(status_code=422, detail="At least one season is required")
    if len(season_list) > 10:
        raise HTTPException(
            status_code=422, detail="At most 10 seasons allowed per request"
        )
...
@router.get(
    "/api/factors/{game_id}", response_model=FactorListResponse, tags=["factors"]
)
...
@router.get(
    "/api/players/search", response_model=List[PlayerSearchResult], tags=["roster"]
)
def search_players(
...
def get_player_weekly_stats(
    ...
    weeks_team: dict = {
        r["week"]: r["team_id"] for r in rows if r["team_id"] is not None
    }
    bye_by_team_full = (
        db.get_bye_weeks(season) if rows or selected_team_id is not None else {}
    )
    ...
    for w in range(1, max_week + 1):
        ...
        if r is not None:
            ...
            week_bye = (
                bye_by_team_full.get(week_team) if week_team is not None else bye_week
            )
            cells.append(
                PlayerWeekCell(
                    week=w,
...
def get_playoff_picture(year: int, db=Depends(get_db)):
    ...
    for g in games_rows:
        ...
        hs["points_for"] += g["home_score"]
        hs["points_against"] += g["away_score"]
        hs["games_played"] += 1
        aws["points_for"] += g["away_score"]
        aws["points_against"] += g["home_score"]
        aws["games_played"] += 1
    ...
    return {
        ...
        "has_playoff_picture": max_week >= 10,
        **conf_data,
    }
...
def get_value_picks(
    ...
    for row in rows:
        ...
        try:
            ...
            logger.warning(
                "Prediction failed for value pick %s @ %s: %s",
                d.get("away_abbr"),
                d.get("home_abbr"),
                e,
            )
        ...
        picks.append(
            ValuePick(
                ...
                is_adverse_weather=(
                    bool(d["is_adverse"]) if d["is_adverse"] is not None else False
                ),
    ...
    if not rows:
        ...
        note = (
            f"No upcoming games with model-Vegas disagreement ≥ {min_edge * 100:.0f}pp"
        )
    else:
...
@router.get(
    "/api/picks/history", response_model=ValuePickHistoryResponse, tags=["predictions"]
)
    ...
    for r in rows:
        ...
        picks.append(
            ValuePickHistoryItem(
                ...
                edge_side="home" if edge > 0 else "away",
                vegas_spread=(
                    float(d["opening_spread"])
                    if d["opening_spread"] is not None
                    else None
                ),
