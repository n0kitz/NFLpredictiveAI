...
def _vegas_team_total(
    spread: Optional[float], over_under: Optional[float], is_home: bool
) -> float:
...
def build_player_feature_vector(
    ...
    if opponent_team_id:
        ...
        adv = db.get_advanced_stats(opponent_team_id, season) or db.get_advanced_stats(
            opponent_team_id, season - 1
        )
        if adv:
...
def build_training_rows(
    db, seasons: List[int], position: str
