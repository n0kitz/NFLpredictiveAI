...
class TestStartSitRecommendation:
    def test_higher_projected_player_starts(self, scorer, mock_db):
        def get_stats(pid, season=None):
            ...
            return {
                "games_played": 16,
                "fantasy_points_ppr": 160.0,
                "fantasy_points_standard": 140.0,
                "targets": 0,
            }
        ...
        def get_player(pid):
            ...
            return {
                "player_id": pid,
                "full_name": names.get(pid, "Unknown"),
                "position": "QB",
                "headshot_url": None,
            }
...
class TestAnalyzeTrade:
    def test_verdict_win_when_get_much_higher(self, scorer, mock_db):
        def get_stats(pid, season=None):
            if pid in (1, 2):  # give side: low value
                return {
                    "games_played": 16,
                    "fantasy_points_ppr": 80.0,
                    "fantasy_points_standard": 70.0,
                    "targets": 0,
                }
            # get side: high value
            return {
    ...
    def test_fair_verdict_when_equal(self, scorer, mock_db):
        mock_db.get_player_stats.return_value = {
            ...
            "fantasy_points_standard": 180.0,
            "targets": 0,
        }
        mock_db.get_player_by_id.return_value = {
            "player_id": 1,
            "full_name": "Equal Player",
            "position": "WR",
            "headshot_url": None,
        }
        mock_db.fetchone.return_value = None
...
class TestWithRealDB:
    def test_matchup_score_real_teams(self, real_scorer, real_db):
        ...
        score = real_scorer.calculate_matchup_score(1, teams[1]["team_id"], "QB", 2024)
    ...
    def test_projection_real_player(self, real_scorer, real_db):
        ...
        row = real_db.fetchone("""SELECT p.player_id FROM players p
...
class TestVBD:
    ...
    def test_top_player_has_positive_vbd(self, real_scorer):
        ...
        for pos in ("RB", "WR"):
            ...
            assert (
                top["vbd"] >= 0
            )  # could be 0 if dataset is sparse, but never negative
