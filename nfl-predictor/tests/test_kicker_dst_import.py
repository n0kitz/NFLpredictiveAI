...
class TestKickerFantasyPoints:
    def test_fg_under_50_scores_3(self):
        assert kicker_fantasy_points(fg_0_39=1, fg_40_49=1, fg_50_plus=0, xp_made=0) == 6.0

    def test_fg_50_plus_scores_5(self):
        assert kicker_fantasy_points(fg_0_39=0, fg_40_49=0, fg_50_plus=2, xp_made=0) == 10.0

    def test_xp_scores_1(self):
        assert kicker_fantasy_points(fg_0_39=0, fg_40_49=0, fg_50_plus=0, xp_made=4) == 4.0

    def test_combined(self):
        # 2×3 + 1×5 + 3×1 = 14
        assert kicker_fantasy_points(fg_0_39=1, fg_40_49=1, fg_50_plus=1, xp_made=3) == 14.0


def _kicker_df(rows):
    """Minimal stats_player_week-shaped frame for kicker tests."""
    defaults = {
        'season_type': 'REG', 'position': 'K',
        'player_display_name': 'Test Kicker', 'team': 'KC', 'opponent_team': 'LV',
        'season': 2025, 'week': 1,
...
class TestDstPointsAllowedScore:
    @pytest.mark.parametrize("pa,expected", [
        (0, 10), (1, 7), (6, 7), (7, 4), (13, 4), (14, 1),
        (20, 1), (21, 0), (27, 0), (28, -1), (34, -1), (35, -4), (50, -4),
    ])
...
class TestDstFantasyPoints:
    def test_event_scoring(self):
        ...
        pts = dst_fantasy_points(
            sacks=3, interceptions=2, fumbles_recovered=1,
            tds=1, safeties=1, blocks=1, points_allowed=0,
        )
        assert pts == 29.0
...
def _dst_df(rows):
    ...
    defaults = {
        'season_type': 'REG', 'position': 'LB', 'team': 'KC',
        'opponent_team': 'LV', 'season': 2025, 'week': 1,
        'def_sacks': 0.0, 'def_interceptions': 0, 'fumble_recovery_opp': 0,
        'def_tds': 0, 'def_safeties': 0, 'special_teams_tds': 0,
        'fg_blocked': 0, 'pat_blocked': 0,
    }
    return pd.DataFrame([{**defaults, **r} for r in rows])
...
def tmp_db(tmp_path):
    ...
    for t in CURRENT_TEAMS:
        db.insert_team(t.name, t.city, t.conference, t.division,
                       t.abbreviation, t.franchise_id, t.active_from, t.active_until)
    yield db
    db.close()
    ...
    def test_sums_weekly_into_season(self, tmp_db):
        ...
        for week, pts in ((1, 9.0), (2, 12.0), (3, 7.0)):
            tmp_db.upsert_player_weekly_stats({
                'player_id': pid, 'season': 2025, 'week': week,
                'team_id': team['team_id'], 'position': 'K',
                'fantasy_points_ppr': pts, 'fantasy_points_standard': pts,
            })
        tmp_db.commit()
...
def test_weekly_stats_kicker_dst_columns(tmp_db):
    cols = {r['name'] for r in tmp_db.fetchall(
        "PRAGMA table_info(player_weekly_stats)", ()
    )}
    for c in ('fg_made_0_39', 'fg_made_40_49', 'fg_made_50_plus', 'fg_missed',
              'xp_made', 'xp_missed', 'dst_sacks', 'dst_interceptions',
              'dst_fumbles_recovered', 'dst_tds', 'dst_safeties', 'dst_blocks',
              'dst_points_allowed'):
