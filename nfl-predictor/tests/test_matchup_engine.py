...
class TestMatchupGrade:
    def test_grade_is_letter(self):
        db = _grade_db()
        with patch(_PATCH_DVP, return_value=11.0), patch(
            _PATCH_PACE, return_value=1.0
        ), patch(_PATCH_PROE, return_value=0.0), patch(_PATCH_RANK, return_value=16):
    ...
    def test_explanation_contains_grade(self):
        db = _grade_db()
        with patch(_PATCH_DVP, return_value=9.5), patch(
            _PATCH_PACE, return_value=1.0
        ), patch(_PATCH_PROE, return_value=0.0), patch(_PATCH_RANK, return_value=16):
    ...
    def test_high_dvp_gives_better_grade(self):
        ...
        with patch(_PATCH_DVP, return_value=high_dvp), patch(
            _PATCH_PACE, return_value=1.15
        ), patch(_PATCH_PROE, return_value=0.1), patch(_PATCH_RANK, return_value=1):
            result = matchup_grade(db, 1, 99, "WR", 2024, 10)
    ...
    def test_low_dvp_gives_worse_grade(self):
        ...
        with patch(_PATCH_DVP, return_value=low_dvp), patch(
            _PATCH_PACE, return_value=0.85
        ), patch(_PATCH_PROE, return_value=-0.1), patch(_PATCH_RANK, return_value=32):
            result = matchup_grade(db, 1, 99, "WR", 2024, 10)
    ...
    def test_rb_benefits_from_run_heavy_script(self):
        ...
        dvp = _AVG_DVP["RB"]
        db = _grade_db()
        with patch(_PATCH_DVP, return_value=dvp), patch(
            _PATCH_PACE, return_value=1.0
        ), patch(_PATCH_PROE, return_value=0.15), patch(_PATCH_RANK, return_value=16):
    ...
    def test_result_contains_all_expected_keys(self):
        db = _grade_db()
        with patch(_PATCH_DVP, return_value=8.0), patch(
            _PATCH_PACE, return_value=1.0
        ), patch(_PATCH_PROE, return_value=0.0), patch(_PATCH_RANK, return_value=16):
